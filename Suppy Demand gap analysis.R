...
Uber <- read.csv("C:/Srungeer Personal/Business/Data analytics - IIITB and Upgrad/Practice Files and Assignments/6. Uber Supply Demand Gap case study/Input files/Uber Request Data.csv", stringsAsFactors = F)

########################################### Data Cleaning ####################################################

#1 Checking for unnecessary rows

head(Uber) # No header rows
tail(Uber) # No footer or total/subtotal rows


#2 Checking for unnecessary columns

str(Uber) # All columns contain relevant information

Uber$Pickup.point <- as.factor(Uber$Pickup.point) #Pickup point has only two unique values so better to store as factor
Uber$Status <- as.factor(Uber$Status) #Status has only three unique values so better to store as factor

str(Uber) # Pickup point and status are now factors


#3 Checking for missing values

sum(is.na(Uber)) #plenty of missing values

Uber[which(is.na(Uber$Driver.id)), ] #Driver ID and drop time is NA when there are no cars available; valid NA
Uber[which(is.na(Uber$Drop.timestamp)), ] #drop time NA when trip is cancelled or there are no cars available; valid NA
...
Uber$Request.timestamp <- str_replace_all(Uber$Request.timestamp, pattern = "/", replacement = "-")
Uber$Drop.timestamp <- str_replace_all(Uber$Drop.timestamp, pattern = "/", replacement = "-")
...
Uber$Request.timestamp[which(nchar(Uber$Request.timestamp) != 19)] <- paste(Uber$Request.timestamp[which(nchar(Uber$Request.timestamp) != 19)], ":00", sep = "")
Uber$Drop.timestamp[which(nchar(Uber$Drop.timestamp) != 19)] <- paste(Uber$Drop.timestamp[which(nchar(Uber$Drop.timestamp) != 19)], ":00", sep = "")
...
sum(duplicated(Uber)) #Return 0 i.e there are no duplicate rows

############################################# Derived Metrics ###############################################

#1. Extracting request.dates from request.timestamp

Uber$request.dates <- format(Uber$Request.timestamp, "%d")
Uber$request.dates <- as.integer(Uber$request.dates)
...
Uber$request.hours <- format(Uber$Request.timestamp, "%H")
Uber$request.hours <- as.integer(Uber$request.hours)
...
Uber <- mutate(Uber, time.slot = derivedFactor(
              "late night" = Uber$request.hours %in% c(0:3),
              "morning" = Uber$request.hours %in% c(4:7),
              "late morning" = Uber$request.hours %in% c(8:12),
              "afternoon" = Uber$request.hours %in% c(13:16),
              "evening" = Uber$request.hours %in% c(17:20),
              "night" = Uber$request.hours %in% c(21:23)
              ))
...
summary(factor(weekdays(Uber$Request.timestamp))) # Only weekday information is present

# No, of days

summary(factor(format(Uber$Request.timestamp, format = "%d"))) # 5 days worth of data has been given

# No. of drivers

n_distinct(Uber$Driver.id, na.rm = T) # Rides information of 300 drivers in present
...
ggplot(Uber, aes(x = Pickup.point)) + 
      geom_bar() + 
...
#A. If a dedicated airport to city fleet is implemented, Uber can artificially increase the no of cars 
